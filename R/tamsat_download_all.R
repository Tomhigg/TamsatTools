...
tamsat_all_download <- function(type, period, outlocation, unZip){
  ...
  if(any(type == c("e", "a", "both"))==FALSE) {stop("type must be a, e, or both")  }
  #check that period is ok
  if(any(period == c("d", "m", "s", "all"))==FALSE) {stop("period must be d, m, s, or all")  }
  #set outlocation if missing
  if(missing(outlocation)==TRUE)  {outlocation <- getwd()}
